...
button.addEventListener('click', () => {
  fetch('https://jsonplaceholder.typicode.com/users')
    .then(
      (respuesta) =>
        respuesta.ok ? Promise.resolve(respuesta) : Promise.reject(respuesta)
      //compruebo que la respuesta es correcta y guardo el resultado en la promise.resolve
    )
    .then((respuesta) => respuesta.json())
    //transformo la respuesta que viene un en json en un objeto js
    .then((respuesta) => {
      const lista = document.getElementById('list');
      const fragment = document.createDocumentFragment();
      for (const userInfo of respuesta) {
        const listItem = document.createElement('LI');
        listItem.textContent = `${userInfo.id} - ${userInfo.name}`;
        fragment.append(listItem);
      }
      lista.append(fragment);
      //trabajo con la respuesta interando por los usuarios y mostrando su informacion
    });
});
